...
- 截图先读取左上角 K 线周期；默认自动识别，手动选择不一致时停止画线识别，支持 1m–1w 周期切换
- 独立 K 线预览页自动定位趋势线两个锚点，预览只连接 P1–P2，不向外延长，并支持缩放和左右移动
...
2. 在“入场线”卡片粘贴图表截图，默认读取左上角周期，也可手动选择周期和线型，然后识别。
...
策略计算接口中的趋势线在两个锚点外按同一斜率延伸；独立 K 线预览页为避免视觉误导，只显示两个锚点之间的线段。创建接口会获取实时合约价并验证止损方向。
